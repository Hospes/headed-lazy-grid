...
internal fun measureLazyGrid(
    ...
    if (itemsCount <= 0) {
        ...
        val minOffset = (-beforeContentPadding + if (spaceBetweenLines < 0) spaceBetweenLines else 0).coerceAtLeast(0)
        ...
        return LazyGridMeasureResult(
            ...
            viewportStartOffset = -beforeContentPadding,
            viewportEndOffset = mainAxisAvailableSize + afterContentPadding,
            visibleItemsInfo = positionedItems.fastFilter {
                (it.index >= visibleLines.first().items.first().index.value && it.index <= visibleLines.last().items.last().index.value) ||
                        it === headerItem
            },
